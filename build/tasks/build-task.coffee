  ...
  grunt.registerTask 'build', 'Build the application', ->
    ...
    if process.platform is 'darwin'
      ...
      if process.platform is 'win32'
        fs.renameSync path.join(shellAppDir, 'electron.exe'), path.join(shellAppDir, 'monitor2.exe')
      else
        fs.renameSync path.join(shellAppDir, 'electron'), path.join(shellAppDir, 'monitor2')
    ...
    mkdir path.resolve(appDir, '..', 'new-app')
    ...
    if process.platform is 'win32'
      cp path.join('resources', 'win', 'monitor2.cmd'), path.join(shellAppDir, 'resources', 'cli', 'monitor2.cmd')
