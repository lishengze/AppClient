...
module.exports = (grunt) ->
  ...
  grunt.registerTask 'codesign:exe', 'Codesign monitor2.exe and Update.exe', ->
    done = @async()
    spawn {cmd: 'taskkill', args: ['/F', '/IM', 'monitor2.exe']}, ->
      cmd = process.env.JANKY_SIGNTOOL ? 'signtool'
      atomExePath = path.join(grunt.config.get('atom.shellAppDir'), 'monitor2.exe')
